...
final class ProjectManager: @unchecked Sendable {
    ...
    init(logs: LogBroker) {
        ...
        load()
        repairLegacyPrivatePathArtifacts()
    }
    ...
    /// Detects when an older JavaScript project actually depends on Node.js.
    /// This is used to migrate projects created by earlier Nexora builds where NodeMobile
    /// was not yet embedded, so existing Discord bots do not stay stuck on JavaScriptCore.
    func recommendedRuntime(_ projectID: String) -> RuntimeKind? {
        guard let project = get(projectID) else { return nil }
        if project.runtime == .typescript { return .typescript }
        guard project.runtime == .javascript else { return nil }

        let workspace = workspaceURL(projectID)
        if FileManager.default.fileExists(atPath: workspace.appendingPathComponent("package.json").path) {
            return .node
        }
        guard let data = try? Data(contentsOf: workspace.appendingPathComponent(project.entrypoint)),
              let source = String(data: data, encoding: .utf8) else { return nil }
        let markers = [
            "require(", "module.exports", "process.env", "process.on(",
            "Buffer.", "__dirname", "__filename", "discord.js", "node:"
        ]
        if markers.contains(where: { source.contains($0) }) { return .node }
        return nil
    }
    ...
    func listFiles(projectID: String, path: String = "") throws -> [ProjectFileInfo] {
        ...
        return try FileManager.default.contentsOfDirectory(
            ...
            let rel = try relativePath(for: url, under: workspaceURL(projectID))
    ...
    func duplicatePath(projectID: String, path: String) throws -> String {
        ...
        try FileManager.default.copyItem(at: source, to: target)
        touch(projectID)
        let newRelative = try relativePath(for: target, under: workspaceURL(projectID))
    ...
    func allFilePaths(projectID: String, limit: Int = 500) throws -> [String] {
        guard get(projectID) != nil else { throw ProjectError.notFound }
        let root = workspaceURL(projectID)
    ...
    struct TextFileChange: Sendable {
        let path: String
        let content: String
    }

    /// Applies a group of text-file edits as one transaction from the caller's point of view.
    /// Every path and payload is validated before the first project file is touched. If a write
    /// fails, the original workspace is restored from an internal temporary snapshot.
    func applyTextFilesAtomically(projectID: String, changes: [TextFileChange]) throws {
        guard get(projectID) != nil else { throw ProjectError.notFound }
        guard !changes.isEmpty else { return }

        var prepared: [(relative: String, data: Data)] = []
        var seen = Set<String>()
        var total = 0
        for change in changes {
            let relative = try sanitize(relativePath: change.path)
            guard !seen.contains(relative) else { throw ProjectError.invalidPath }
        ...
        let workspace = workspaceURL(projectID)
        let transactionRoot = projectURL(projectID)
            .appendingPathComponent(".transaction-\(UUID().uuidString)", isDirectory: true)
        let snapshot = transactionRoot.appendingPathComponent("before", isDirectory: true)
        ...
        try FileManager.default.copyItem(at: workspace, to: snapshot)
        try FileManager.default.createDirectory(at: staged, withIntermediateDirectories: true)

        do {
            for item in prepared {
                let temp = staged.appendingPathComponent(item.relative)
                try FileManager.default.createDirectory(at: temp.deletingLastPathComponent(), withIntermediateDirectories: true)
                try item.data.write(to: temp, options: .atomic)
            }

            for item in prepared {
                let source = staged.appendingPathComponent(item.relative)
                let target = workspace.appendingPathComponent(item.relative)
                try FileManager.default.createDirectory(at: target.deletingLastPathComponent(), withIntermediateDirectories: true)
            ...
            try? FileManager.default.removeItem(at: workspace)
            try FileManager.default.copyItem(at: snapshot, to: workspace)
            throw error
        }

        touch(projectID)
        for item in prepared {
            logs.append(projectID: projectID, "Updated file \(item.relative)")
        }
    }

    /// Cleans only the zero-byte legacy artifacts created by the old `/private/var` path bug.
    /// A file is removed only when stripping one or more leading "private" prefixes points to an
    /// existing sibling, so legitimate files named `private*.js` are left alone.
    private func repairLegacyPrivatePathArtifacts() {
        for project in list() {
            let workspace = workspaceURL(project.id)
            guard let enumerator = FileManager.default.enumerator(
                at: workspace,
            ...
            for case let url as URL in enumerator {
                ...
                var stripped = false
                while candidate.lowercased().hasPrefix("private") && candidate.count > "private".count {
                    candidate.removeFirst("private".count)
                    stripped = true
                }
                guard stripped, !candidate.isEmpty else { continue }
    ...
    private func relativePath(for url: URL, under root: URL) throws -> String {
        ...
        let childParts = canonicalURL.pathComponents
        guard childParts.count >= rootParts.count,
              Array(childParts.prefix(rootParts.count)) == rootParts else {
            throw ProjectError.invalidPath
        }
        let relative = childParts.dropFirst(rootParts.count).joined(separator: "/")
        guard !relative.isEmpty else { throw ProjectError.invalidPath }
        return relative
    }
