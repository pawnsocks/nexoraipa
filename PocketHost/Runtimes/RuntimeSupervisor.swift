...
final class RuntimeSupervisor: @unchecked Sendable {
    ...
    func statuses() -> [RuntimeStatus] {
        RuntimeKind.allCases.map { kind in
            ...
            case .typescript:
                return .init(id: kind, available: false, detail: "Experimental · local TypeScript transpiler not bundled yet")
            case .node:
                return .init(id: kind, available: false, detail: "Not available on iOS · full desktop Node.js")
            case .bun:
    ...
    func run(kind: RuntimeKind, code: String) -> RuntimeResult {
        ...
        case .python:
            return runPython(code, projectID: nil)
        case .lua:
    ...
    private func runPython(_ code: String, projectID: String?) -> RuntimeResult {
        ...
        guard let binary = pythonFrameworkBinary else {
            return .init(output: "Python is not bundled in this IPA yet. Nexora will not pretend it is installed.", succeeded: false)
        }
