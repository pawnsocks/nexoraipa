                    ...
                    if model.localProjects.isEmpty {
                        VStack(spacing: 10) {
                            ...
                            Text("No projects yet")
                                .font(.headline)
                            Text("Create a local Node.js, TypeScript, JavaScript, Python or Lua project.")
                                .font(.caption)
...
private enum NexoraCreatePathKind: String, Identifiable {
    case file
    case folder
...
struct NexoraLocalFilesView: View {
    ...
    @State private var createKind: NexoraCreatePathKind?
    ...
    var body: some View {
        ...
        .sheet(item: $createKind) { kind in
            NavigationStack {
                Form {
                    TextField(kind == .file ? "name.ext" : "Folder name", text: $newName)
                        .textInputAutocapitalization(.never)
                        .autocorrectionDisabled()
                }
                .navigationTitle(kind == .file ? "New File" : "New Folder")
                .toolbar {
                    ToolbarItem(placement: .cancellationAction) {
                        Button("Cancel") { createKind = nil }
                    }
                    ToolbarItem(placement: .confirmationAction) {
                        Button("Create") {
                            if kind == .file { createFile() } else { createFolder() }
                            if status.isEmpty { createKind = nil }
                        }
    ...
    private func createFile() {
        ...
        do {
            ...
            status = "Could not create file: \(error.localizedDescription)"
        }
    }
