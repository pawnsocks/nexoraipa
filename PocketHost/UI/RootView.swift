        ...
        NavigationStack {
            List {
                Section("Device") {
                    ...
                    LabeledContent("Background hosting", value: model.backgroundHostingEnabled ? "On" : "Off")
                    Text(model.backgroundHostingStatus)
                        .font(.caption)
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section("Background Hosting") {
                    Toggle(
                        "Keep running when possible",
                        isOn: Binding(
                            get: { model.backgroundHostingEnabled },
                            set: { model.setBackgroundHostingEnabled($0) }
                        )
                    )
                    LabeledContent("Status", value: model.backgroundHostingStatus)
                    Text("On iOS/iPadOS 26+, Nexora requests Continued Processing for projects you start with Run and shows system progress while the app is backgrounded. Older iOS versions use the shorter background-time fallback. iOS can still stop background work under resource pressure, and force-quitting Nexora stops hosting.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
