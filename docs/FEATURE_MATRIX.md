...
| Python | Yes in IPA pipeline | Official CPython 3.14.7 is built and embedded by GitHub Actions; runtime status also checks the stdlib is present. |
| TypeScript | Yes in IPA pipeline | Runs through embedded NodeMobile 24 using Node type stripping. |
| WASM/WASI | No | Capability path documented; runtime pack execution not complete. |
| Node.js | Yes in IPA pipeline | Embedded NodeMobile 24.18.0-0; project logs, stop marker, package auto-install for pure-JS npm dependencies. |
| Bun | Blocked | Native Bun is not available in this iOS build. |
| Docker | Blocked | Docker is not available on normal iOS. |
| Run/Stop/Restart | Yes | JavaScript/Lua plus Node/TypeScript lifecycle; Python execution for scripts. |
